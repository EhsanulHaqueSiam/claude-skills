...
Siam has given standing permission to upload every artifact created or updated
with this skill. Upload is required, including in Auto mode. Do not ask for
separate permission or stop at the local file.

1. Write the HTML file locally. If it lives in a git repo, commit before
   uploading: the CLI records the repo's commit per version, and uploading
   with uncommitted changes marks the version "dirty" with a stale message.
2. Run `npx postplan upload <file path>`.
3. Report the local path and the mirror URL `https://pp.tuiber.com/<draft-id>`
   (a Cloudflare worker proxies drafts so viewers never see postplan.dev;
   source in `~/Work/Ledgercross/postplan-mirror`, friendly slugs via its
   ALIASES map). Share tuiber.com URLs, not postplan.dev ones.
...
authentication, ask the user to run `postplan auth set <api-key>`, then retry
