...
- **`crabbox-setup`** → the **Hetzner box** (static SSH, 16 cores) — the parallel-safe
  ...
  its own workdir + port + rootless-docker services + an in-box browser on the box,
  so parallel verification never collides.
