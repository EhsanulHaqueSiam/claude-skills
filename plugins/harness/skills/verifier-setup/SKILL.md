...
- **Sandbox (crabbox)** — an isolated stack per agent on the Hetzner box, for
  **concurrent** loops or a fixed-port/single-instance stack. If chosen and not yet
  set up, scaffold via **`crabbox-setup`** (static SSH, no Daytona); the generated
  skill drives the app in-box via `cbx.sh pw`.
